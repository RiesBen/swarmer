...
# return 
    ...
    out = seg_intersect(a1,a2,b1,b2)
    if np.any(np.isnan(out)):
        print("here")
        return True #FIXME
    print(out)
    if out[0] <= max(a1[0],a2[0]) and out[0] >= min(a1[0],a2[0]) and out[1] <= max(a1[1], a2[1]) and out[1] >= min(a1[1],a2[1]) :
        print("there")
        return True
    else : return False
"""

def plot_graph(G, colors, sizes, labels,edge_colors):

    pos=nx.get_node_attributes(G,'pos')

    edge_labels = nx.get_edge_attributes(G,'weight')
    nx.draw_networkx(G, pos,node_color = colors, node_size = sizes, labels = labels, font_size = 8,edge_color=edge_colors)
